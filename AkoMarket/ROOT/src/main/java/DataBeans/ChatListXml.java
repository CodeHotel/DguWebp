...
public class ChatListXml {
    ...
    @XmlElement public int user_read;
    ...
    public ChatListXml(){}
    public ChatListXml(int id, int user1, int user2, int user_read, int user2_read, String last_msg, int last_idx, String time) {
        this.id = id;
        this.user1 = user1;
        this.user2 = user2;
    ...
    public ChatListXml(Chatlist fromChatList,int uid) {
        this.id = fromChatList.id;
        this.user1 = fromChatList.user1.getUid();
        this.user2 = fromChatList.user2.getUid();
        this.user_read = uid==user1? fromChatList.user1_read : fromChatList.user2_read;
    ...
    public static ChatListXml[] ChatListConvert(Chatlist[] from, int uid){
        ChatListXml[] ret = new ChatListXml[from.length];
        for(int i=0; i< from.length; i++){
            ret[i] = new ChatListXml(from[i], uid);
        }
        return ret;
    }
}
